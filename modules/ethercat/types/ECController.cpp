...
namespace forte::eclipse4diac::io::ethercat {
  ...
  namespace {
    const auto cDataInputNames = std::array{"QI"_STRID, "Enable"_STRID, "ControllerIndex"_STRID, "UpdateInterval"_STRID};
  ...
  void FORTE_ECController::readInputData(const TEventID paEIED) {
    switch (paEIED){
      ...
      case scmEventREQID:
        ...
        readData(2, var_ControllerIndex, conn_ControllerIndex);
  ...
  CIEC_ANY *FORTE_ECController::getDI(const size_t paIndex) {
    switch (paIndex) {
      ...
      case 2: return &var_ControllerIndex;
  ...
  CDataConnection **FORTE_ECController::getDIConUnchecked(const TPortId paIndex) {
    switch (paIndex) {
      ...
      case 2: return &conn_ControllerIndex;
  ...
  void FORTE_ECController::setConfig() {
    ...
    config.mECControllerId = var_ControllerIndex.operator TForteUInt16();
